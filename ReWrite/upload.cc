...
class Upload{
  ...
  private:
    int MatchBoundry(char* buf,int blen,int* boundryPos){
      ...
      if(!memcmp(buf,_f_boundry.c_str(),_f_boundry.size())){
        boundryPos = 0;
      ...
      for(int i = 0;i<blen;++i){
       ...
       // if((size_t)(blen-i)>_m_boundry.size()){
          if(!memcmp(buf+i,_m_boundry.c_str(),_m_boundry.size())){
            *boundryPos = i;
            return BOUNDRY_MIDDLE;
          }
          else if(!memcmp(buf+i,_l_boundry.c_str(),_l_boundry.size())){
            *boundryPos = i;
            return BOUNDRY_LAST;
          }
       // }
    ...
    bool GetFileName(char *buf,int * content_pos){
      ...
      *content_pos = ptr-buf+4;
      ...
      if(pos==std::string::npos)
        return false;
      _file_name  = header.substr(pos+file_sep.size());
      pos = _file_name.find("\"");
      if(pos==std::string::npos)
        return false;
